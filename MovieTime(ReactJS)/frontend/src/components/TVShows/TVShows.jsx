...
const TVShows = ({ filterOpen, searchValue }) => {
  ...
  const handleCounterClick = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setRecommendationError(
          "Nije moguće dobiti preporuke. Pokušajte ponovo kasnije."
        );
      }
    } catch (error) {
      setRecommendationTVShows([]);
      setRecommendationError(
        "Greška u pribavljanju preporuka: " + error.response.data
      );
  ...
  const fetchFavouriteTVShows = async (userEmail) => {
    try {
      const favoriteTVShowsResponse = await axios.get(
        `http://localhost:5023/User/GetFavoriteTVShows/${userEmail}`
      );
      ...
      try {
        ...
        if (isSorted) {
          if (sortValue.value === "asc" || sortValue.value === "desc") {
            url = `http://localhost:5023/TVShow/GetTVShowAlphabeticalOrder/${
              sortValue.value === "asc" ? true : false
            }/${currentPage}`;
          } else {
            url = `http://localhost:5023/TVShow/GetTVShowYearOrder/${
              sortValue.value === "yearLatest" ? true : false
            }/${currentPage}`;
          }
        } else {
        ...
        if (filteredTVShows.length > 0) {
          ...
          const newFiltered = listSearchedTVShows.filter((tvShow) =>
            prevFilteredTVShows.some(
              (filteredTVShow) => filteredTVShow.name === tvShow.name
            )
          );
    ...
    const fetchSearchTVShows = async () => {
      try {
        const searchResponse = await axios.get(
          `http://localhost:5023/TVShow/GetTVShowSearch/${searchValue}/${currentPage}`
        );
        if (filteredTVShows.length > 0) {
          const listSearchedTVShows = searchResponse.data;
          const newFiltered = listSearchedTVShows.filter((tvShow) =>
            filteredTVShows.some(
              (filteredTVShow) => filteredTVShow.name === tvShow.name
            )
          );
          setFilteredTVShows(newFiltered);
  ...
  const handleAddToFavourites = async (tvShowName, email) => {
    try {
      const addToFavouritesResponse = await axios.post(
        `http://localhost:5023/User/AddFavoriteTVShow/${tvShowName}/${email}`
      );
      if (addToFavouritesResponse.status === 200) {
        console.log("Proso sam");
        setFavoriteTVShows([...favoriteTVShows, tvShowName]);
        alert(
          `Serija ${tvShowName} je uspešno dodata u listu omiljenih serija.`
        );
        setOverlayVisible(false);
  ...
  const handleRemoveFromFavorites = async (tvShowName, email) => {
    try {
      const removeFromFavoritesResponse = await axios.post(
        `http://localhost:5023/User/RemoveFavoriteTVShow/${tvShowName}/${email}`
      );
      if (removeFromFavoritesResponse.status === 200) {
        const newFavoriteTVShows = favoriteTVShows.filter(
          (tvShow) => tvShow.name !== tvShowName
        );
        ...
        setCurrentPage(1);
        alert(
          `Serija ${tvShowName} je uspešno uklonjena sa liste omiljenih serija.`
        );
        setOverlayVisible(false);
      } else {
      ...
      console.error(
        "Greska pri uklanjanju serije iz liste omiljenih serija.",
        error
      );
    }
  };
  ...
  const handleRating = async (tvShowName, rating) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:5023/User/RateTVShow/${tvShowName}/${userEmail}/${newRating}/OK`
      );
      if (response.status === 200) {
        setTvShows((prevTVShows) =>
          prevTVShows.map((tvShow) =>
            tvShow.name === tvShowName ? { ...tvShow, rating: rating } : tvShow
          )
        );
    ...
    <>
      <div className={!filterOpen ? "sekcije" : `${styles.filterSekcije}`}>
        ...
        <section
          id="tvshows"
          className={
            tvShowsToDisplay.length > 0 ? "" : `${styles.emptySection}`
          }
        >
                ...
                <div className="menu-container">
                  {tvShowsToDisplay.map((tvShow, index) => (
                    <div
                      ...
                      key={index}
                      onClick={() => handleImageClick(tvShow)}
                    >
                      <Image
                        alt={tvShow.name}
                        src={
                          tvShow.image
                            ? `data:image/jpeg;base64,${tvShow.image}`
                            : ""
                        }
                        style={{
                          width: "200px",
                ...
                {recommendationTVShows.length > 0 ? (
                  <>
                    <div className={`${styles.programOptionsContainer}`}>
                      <div className="home-text">
                        <p>
                          <strong>{recommendationType}:</strong>
                        </p>
                      </div>
                    </div>
                    <div className="menu-container">
                ...
                <>
                  {location.pathname === "/user" ? (
                    ...
                    >
                      <div className="home-text">
                        <h2>
                          <strong>Stigli ste do kraja</strong>
                        </h2>
                      </div>
                      <Button
                        padding={3}
                    ...
                    >
                      <div className="home-text">
                        <h2>
                          <strong>Lista omiljenih serija je prazna</strong>
                        </h2>
                      </div>
                    </Box>
                  )}
          ...
          <DialogRoot placement="center" open={overlayVisible} size="sm">
            ...
            >
              <DialogBody pt="4">
                ...
                >
                  {selectedTVShow.link &&
                  getYouTubeVideoId(selectedTVShow.link) ? (
                    <iframe
                ...
                <Box display="flex" justifyContent="space-between" mb="4">
                  <Box>
                    <strong>Godina produkcije:</strong>{" "}
                    {selectedTVShow.yearOfRelease}
                  </Box>
                  <Box>
                ...
                <Box mb="4">{selectedTVShow.description}</Box>
                    ...
                    {location.pathname !== "/user-favourite-tvShows" ? (
                      <>
                        <Button
                          ...
                          disabled={favoriteTVShows.some(
                            (tvShow) => tvShow.name === selectedTVShow.name
                          )}
                        >
                        ...
                        <Rating
                          ...
                          mr={3}
                          onChange={(newRating) =>
                            handleRating(selectedTVShow.name, newRating)
                          }
                        />
                      </>
                    ) : (
                      ...
                      >
                        <RiDislikeFill style={{ color: "red" }} /> Ukloni iz
                        omiljenih
                      </Button>
                    )}
                  </HStack>
